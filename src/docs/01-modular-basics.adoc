...
[.lead]
https://maven.apache.org/[Apache Maven] 4 introduces a new way to structure multi-module Java projects using the _module source hierarchy_.
...
The complete source code is available on https://github.com/aschemaven/maven-modular-sources-showcases[GitHub] – clone it and follow along.
...
Over time, tools like https://www.archunit.org/[ArchUnit] emerged.
...
Java 9 introduced _https://openjdk.org/jeps/261[Java Modules]_, providing modularization directly at the language level.
...
Some people also refer to it as _JPMS_, but this was never an official abbreviation and  the main author, Mark Reinhold, https://www.youtube.com/watch?v=ny4CqBX_kaQ&t=2062s[heavily discourages using it].
Java 9 introduced JPMS as part of https://openjdk.org/projects/jigsaw/[Project Jigsaw].
You may therefore also find references to Jigsaw in related documents and samples.

The following terms are essential when working with Java Modules:

`module`:: A named, self-describing collection of packages with explicit dependencies and exports.
`requires`:: Declares a dependency on another module.
`exports`:: Makes a package accessible to other modules.
`module-info.java`:: The module descriptor file that defines the module's name, dependencies, and exports.
====
...
====
Both Maven 3 and https://docs.gradle.org/current/samples/sample_java_modules_multi_project.html[Gradle] support compiling and running Java modules.
...
We split the application into two Java modules: a *core* module containing the domain model and analysis services, and a *cli* module providing a command-line interface using https://picocli.info/[picocli].
Both modules use https://logging.apache.org/log4j/[Log4j 2] for logging, which is itself a fully modular library.

image::images/module-structure.svg[Module structure]

For readability, the diagram uses shortened module names (`analyzer.cli`, `analyzer.core`) corresponding to the fully qualified modules `net.aschemann.maven.demos.analyzer.cli` and `net.aschemann.maven.demos.analyzer.core`.
...
----
        <sources>
            <source>
                <module>net.aschemann.maven.demos.analyzer.core</module> <!--1-->
            </source>
            <source>
                <module>net.aschemann.maven.demos.analyzer.cli</module> <!--2-->
            </source>
        </sources>
----
...
pom.xml <1>
src/
├── net.aschemann.maven.demos.analyzer.core/ <2>
│   └── main/java/
│       ├── module-info.java
│       └── net/aschemann/maven/demos/analyzer/core/
└── net.aschemann.maven.demos.analyzer.cli/ <3>
    └── main/java/
        ├── module-info.java
        └── net/aschemann/maven/demos/analyzer/cli/
----
<1> The _one and only_ Maven POM needed for the whole project.
    Maven 4 specific: uses `modelVersion` 4.1.0 and the new `<sources>` element to declare multiple Java modules inside a single Maven project.
    Earlier Maven versions required a separate POM per Java module.
<2> This is the module source directory for the core module
<3> This is the module source directory for the CLI module
...
This file is the module descriptor that defines the module's name, its dependencies, and which packages it exposes to other modules (cf. <<sec:module-dependencies>>).

[NOTE]
.Module Names and Directory Names
====
You'll notice that the module names (e.g., `net.aschemann.maven.demos.analyzer.core`) match the directory names under `src/`.
...
java --module-path "target/classes:target/lib" \#<1>
     --module net.aschemann.maven.demos.analyzer.cli/net.aschemann.maven.demos.analyzer.cli.AnalyzerCommand \#<2>
     README.* #<3>
----
...
https://openjdk.org/jeps/411[Java 17 set them to _deprecated_] and https://openjdk.org/jeps/486[Java 24 removed them entirely].
...
We address both issues in the follow-up homework article on the `blog-1-homework` branch.
